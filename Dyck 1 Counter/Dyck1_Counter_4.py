...
class Net(nn.Module):
    def __init__(self, counter_input_size, counter_output_size, relu_input_size, relu_output_size, output_layer_input_size, output_size):
        ...
        self.l2n1 = nn.Linear(relu_input_size,relu_output_size)
        self.l2n1.weight = nn.Parameter(torch.tensor([1,0],dtype=torch.float32))
        self.l2n1.bias = nn.Parameter(torch.tensor(0,dtype=torch.float32))
        self.relu1 = nn.ReLU()
        self.l2n2 = nn.Linear(relu_input_size,relu_output_size)
        self.l2n2.weight = nn.Parameter(torch.tensor([-1,1],dtype=torch.float32))
        self.l2n2.bias = nn.Parameter(torch.tensor(0,dtype=torch.float32))
        self.relu2 = nn.ReLU()
        self.l3n1 = nn.Linear(output_layer_input_size,output_size)
        self.l3n1.weight = nn.Parameter(torch.tensor([[0,0],[1,1]],dtype=torch.float32))
        self.l3n1.bias = nn.Parameter(torch.tensor([0,0],dtype=torch.float32))
