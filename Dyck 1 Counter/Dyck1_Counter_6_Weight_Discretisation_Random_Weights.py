...
IGNORE, THIS IS NOT CORRECT
"""


document_name = 'Dyck1_Counter_6_Weight Discretisation_Random_initialisation_Softmax_BCE_run_3.txt'
excel_name = 'Dyck1_Counter_6_Weight Discretisation_Random_initialisation_Softmax_BCE_run_3.xlsx'
...
def train():
    for epoch in range(num_epochs):
        ...
        num_correct = 0
        current_loss = 0
        current_loss_no_discretisation = 0
        epochs.append(epoch)
        expected_classes = []
        predicted_classes = []
        ...
        for i in range(len(X_train)):
            ...
            current_loss_no_discretisation += loss.item()
            # if i == len(X_train)-1:
            #     losses_without_discretisation.append(loss)
            weight_sum = torch.tensor(0, dtype=torch.float32)
            for param in model.parameters():
                if param.shape == torch.Size([]):
                    ...
                    for elem in param:
                        ...
                        elem_differences = torch.cat(
                            (elem_floor_difference.unsqueeze(dim=0), elem_ceil_difference.unsqueeze(dim=0)))
        ...
        losses_without_discretisation.append(current_loss_no_discretisation/len(X_train))
    ...
    df1['losses with discretisation'] = all_losses
    df1['losses without discretisation'] = losses_without_discretisation
