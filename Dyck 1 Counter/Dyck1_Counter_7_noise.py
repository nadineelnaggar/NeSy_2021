...
document_name = 'Dyck1_Counter_7_correct_weights_with_noise_Sigmoid_BCE_SGD_run_3.txt'
excel_name = 'Dyck1_Counter_7_correct_weights_with_noise_Sigmoid_BCE_SGD_run_3.xlsx'
...
class Net(nn.Module):
    def __init__(self, input_size,output_size, hidden_size_1, hidden_size_2):
        ...
        self.out.weight = nn.Parameter(torch.tensor([1+random.uniform(min_noise,max_noise),1+random.uniform(min_noise,max_noise)],dtype=torch.float32))
        self.out.bias = nn.Parameter(torch.tensor(0+random.uniform(min_noise,max_noise),dtype=torch.float32))
