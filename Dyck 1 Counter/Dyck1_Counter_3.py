...
class Net(nn.Module):
    def __init__(self, counter_input_size, hidden_size, counter_output_size, output_size):
        ...
        self.out.weight = nn.Parameter(torch.tensor([0,1],dtype=torch.float32))
    ...
    def forward(self,x, counter_rec_input, hidden1_rec_input, hidden2_rec_input, print_flag=False):
        ...
        hidden1_rec_output = hidden1_output
        if print_flag==True:
        ...
        hidden2_rec_output = hidden2_output
        if print_flag==True:
        ...
        return output, counter_rec_output.unsqueeze(dim=0),hidden1_rec_output.unsqueeze(dim=0),hidden2_rec_output.unsqueeze(dim=0)
