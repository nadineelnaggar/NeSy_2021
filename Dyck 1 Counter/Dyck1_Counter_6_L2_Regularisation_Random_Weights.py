...
document_name = 'Dyck1_Counter_6_L2_Regularisation_random_initialisation_Softmax_BCE_run_3.txt'
excel_name = 'Dyck1_Counter_6_L2_Regularisation_random_initialisation_Softmax_BCE_run_3.xlsx'
...
def train():
    for epoch in range(num_epochs):
        ...
        for i in range(len(X_train)):
            ...
            l2_regularisation = torch.tensor([0], dtype=torch.float32)
            for param in model.parameters():
                l2_regularisation += torch.norm(param, 2)
            loss = loss + l2_regularisation
            loss.backward()
            optimiser.step()
